import { auth, currentUser } from "@clerk/nextjs/server";
import { db } from "./prisma";
import crypto from "crypto";

const PLAN_CREDITS = {
  pro: 15,
  starter: 5,
  free: 1,
};

const getCurrentPlan = async () => {
  const { has } = await auth();
  if (has({ plan: "pro" })) return "pro";
  if (has({ plan: "starter" })) return "starter";
  return "free";
};

const shouldAllocateCredits = (dbUser, currentPlan) => {
  // Always allocate if plan changed
  if (dbUser.currentPlan !== currentPlan) return true;

  // Allocate if never allocated before
  if (!dbUser.creditsLastAllocatedAt) return true;

  // Allocate if it's a new calendar month since last allocation
  const now = new Date();
  const last = new Date(dbUser.creditsLastAllocatedAt);
  const isNewMonth =
    now.getFullYear() > last.getFullYear() || now.getMonth() > last.getMonth();

  return isNewMonth;
};

export const checkUser = async () => {
  const user = await currentUser();
  if (!user) return null;

  try {
    const currentPlan = await getCurrentPlan();
    const credits = PLAN_CREDITS[currentPlan];

    const loggedInUser = await db.user.findUnique({
      where: { clerkUserId: user.id },
    });

    if (loggedInUser) {
      const name = `${user.firstName || ""} ${user.lastName || ""}`.trim();
      const hasProfileChanged =
        loggedInUser.name !== name || loggedInUser.imageUrl !== user.imageUrl;

      if (hasProfileChanged) {
        await db.user.update({
          where: { clerkUserId: user.id },
          data: {
            name,
            imageUrl: user.imageUrl,
          },
        });
        // Update local object to reflect what we just saved, before checking credits
        loggedInUser.name = name;
        loggedInUser.imageUrl = user.imageUrl;
      }

      // Interviewers don't have a credit subscription — skip allocation
      if (loggedInUser.role === "INTERVIEWER") return loggedInUser;

      if (shouldAllocateCredits(loggedInUser, currentPlan)) {
        // Check if it's a new month to decide on rollover logic
        let isNewMonth = false;
        if (loggedInUser.creditsLastAllocatedAt) {
          const now = new Date();
          const last = new Date(loggedInUser.creditsLastAllocatedAt);
          isNewMonth =
            now.getFullYear() > last.getFullYear() ||
            now.getMonth() > last.getMonth();
        }

        // If it's explicitly a new month, add the new plan's credits to the previous leftover credits.
        // If it's a mid-month plan switch, just snap to the new plan credits.
        const newCredits = isNewMonth
          ? credits + (loggedInUser.credits ?? 0)
          : credits;

        return await db.user.update({
          where: { clerkUserId: user.id },
          data: {
            credits: newCredits,
            currentPlan,
            creditsLastAllocatedAt: new Date(),
          },
        });
      }

      return loggedInUser;
    }

    // New user — create with credits from their current plan
    const name = `${user.firstName} ${user.lastName}`;
    const randomChars = crypto.randomBytes(3).toString("hex").toUpperCase();
    const referralCode = `PREP-${randomChars}`;

    return await db.user.create({
      data: {
        clerkUserId: user.id,
        name,
        imageUrl: user.imageUrl,
        email: user.emailAddresses[0].emailAddress,
        credits,
        currentPlan,
        creditsLastAllocatedAt: new Date(),
        referralCode,
      },
    });
  } catch (error) {
    console.log("checkUser error:", error.message);
    return null;
  }
};
